...
type TasksDomain = PluginRpcHandlers<typeof tasksRpcContract>;
...
async function runAttach(
  ...
  const result = tasksRpcContract.attachTaskThread.output.parse(
    await domain.attachTaskThread(
      tasksRpcContract.attachTaskThread.input.parse({ taskId: task.id, threadId }),
    ),
  );
  return args.flags.has("json")
    ? json({ task, ...result })
    : `Attached ${result.taskThread.threadId} to ${task.key}`;
}
...
export function registerTasksCli(
  ...
  store: TasksApiStore,
  status: PluginStatus,
  providerDomain?: TasksDomain,
): void {
  const domain = providerDomain ?? registerHandlers(bb, store);
  bb.cli.register({
    name: "tasks",
    summary:
      ...
      try {
        ...
        switch (command) {
          ...
          case "delegate":
            ...
            break;
          case "attach":
            stdout = await runAttach(domain, ctx, rest);
            break;
          case "threads":
