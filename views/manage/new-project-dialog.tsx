...
const DEFAULT_JIRA_JQL = "ORDER BY updated DESC";
...
export function NewProjectDialog({ open, onOpenChange }: NewProjectDialogProps) {
  ...
  const [taskProvider, setTaskProvider] = useState<TaskProviderId>("local");
  const [jiraJql, setJiraJql] = useState(DEFAULT_JIRA_JQL);
  ...
  useEffect(() => {
    ...
    setTaskProvider("local");
    setJiraJql(DEFAULT_JIRA_JQL);
    setError(null);
  }, [open]);
  ...
  const canSubmit =
    ...
    prefix.length > 0 &&
    prefixError === null &&
    (taskProvider !== "beads" || linkedTrimmed !== "") &&
    !submitting;
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <Field
            ...
            hint={
              taskProvider === "local"
                ? "Tasks are stored in this plugin."
                : taskProvider === "beads"
                  ? "Tasks are read from and written to the linked Beads workspace."
                  : "Jira issues are shown as a read-only JQL view."
            }
          >
            <Select
              value={taskProvider}
              onValueChange={(value) =>
                setTaskProvider(value as TaskProviderId)
              }
            >
              <SelectTrigger aria-label="Task source" className="h-8">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="local">Local Tasks</SelectItem>
                <SelectItem value="beads">Beads</SelectItem>
                <SelectItem value="jira">Jira · read-only</SelectItem>
              </SelectContent>
            </Select>
          </Field>
          {taskProvider === "jira" ? (
            <Field label="JQL" hint="The Jira view used by this Tasks project.">
              <Input
                value={jiraJql}
          ...
          <Field
            ...
            hint={
              taskProvider === "beads"
                ? "Required for Beads. The linked project must contain a Beads workspace."
