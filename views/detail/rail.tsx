...
const BEADS_STATUS_LABELS: Record<(typeof BEADS_STATUSES)[number], string> = {
  open: "Open",
...
export interface TaskPropertiesProps {
  ...
  onSetBeadsStatus: (status: (typeof BEADS_STATUSES)[number]) => void;
}
...
function StatusMenu({
  ...
  if (task.sourceId === "jira") {
    return (
      <button
        type="button"
        disabled
        title="Jira tasks are read-only"
  ...
  const statuses =
    task.sourceId === "beads" ? BEADS_STATUSES : TASK_STATUSES;
  const dependencyBlocked =
    task.sourceId === "beads" && task.blockedByTaskIds.length > 0;
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" className={triggerClassName}>
          ...
          {dependencyBlocked
            ...
            : task.sourceId === "beads"
            ? (BEADS_STATUS_LABELS[
                task.nativeStatus as (typeof BEADS_STATUSES)[number]
              ] ?? task.nativeStatus)
      ...
      <DropdownMenuContent align="start">
        {statuses.map((status) => (
          <DropdownMenuItem
            key={status}
            disabled={dependencyBlocked && status === "in_progress"}
            onSelect={() =>
              task.sourceId === "beads"
                ? onSetBeadsStatus(status as (typeof BEADS_STATUSES)[number])
                : onUpdate({ status: status as TaskStatus })
            }
          >
            <StatusIcon
              status={
            ...
            {task.sourceId === "beads"
              ? BEADS_STATUS_LABELS[
                  status as (typeof BEADS_STATUSES)[number]
                ]
...
function ExecutionTargetsMenu({
  ...
  }, [rpc, task.id, task.projectId]);

  const save = async (projectIds: string[]) => {
    if (saving) return;
    setSaving(true);
    try {
      await rpc.call("setTaskExecutionTargets", { taskId: task.id, projectIds });
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" className={triggerClassName} aria-label="Edit execution targets">
          <Icon name="Layers" className="size-3.5 shrink-0" />
          <span className={cn("truncate", selected.length === 0 && "text-muted-foreground")}>
            {selected.length === 0 ? "Choose microservices…" : names}
          </span>
          {selected.length > 1 ? (
      ...
      <PopoverContent align="start" className="w-72 p-0">
        <Command>
          <CommandInput placeholder="Search bb projects…" />
          <CommandList>
            <CommandEmpty>No bb projects found.</CommandEmpty>
            <CommandGroup>
              {projects.map((project) => {
                ...
                return (
                  ...
                  >
                    <Icon name={checked ? "Check" : "Square"} className="size-3.5" />
  ...
  return (
    <aside className={cn("w-56 shrink-0 py-10 pl-2 pr-6", className)}>
      ...
      <StatusMenu
        task={task}
        onUpdate={onUpdate}
        onSetBeadsStatus={onSetBeadsStatus}
        ...
        <DispatchControl
          ...
          blockedReason={
            task.blockedByTaskIds.length > 0
              ? `Blocked by ${task.blockedByTaskIds.length} unresolved task${task.blockedByTaskIds.length === 1 ? "" : "s"}`
              : task.sourceId === "beads" && task.nativeStatus === "blocked"
