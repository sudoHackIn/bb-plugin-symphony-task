...
function TaskDetail({
  ...
  const setBeadsStatus = async (
    status: "open" | "in_progress" | "blocked" | "deferred" | "closed",
  ) => {
    try {
      await providerRpc.call("setBeadsTaskStatus", {
        taskId: task.id,
        projectId: task.projectId,
        status,
      });
      ...
      <div className="flex flex-1 items-stretch rounded-lg border border-border bg-card shadow-2xs">
        <div className="mx-auto w-full min-w-0 max-w-[55rem] flex-1 px-7 pb-16 pt-8 @3xl:px-13 @3xl:pt-11">
          ...
          <InlineProperties
            ...
            onSetBeadsStatus={(status) => void setBeadsStatus(status)}
...
export function ResolvedDetailView({
  taskKey,
  task,
  error,
  onOpenRelatedTask,
