...
function taskDetailSubPath(taskKey: string, projectId?: string): string {
  return tasksRouteToSubPath({
    kind: "task",
    taskKey,
    ...(projectId ? { projectId } : {}),
  });
...
export function TaskDirectiveCard({ attributes }: PluginMessageDirectiveProps) {
  ...
  const openInSidePanel = () => {
    const opened = navigate.openThreadPanel({
      actionId: "task",
      title: task.key,
      params: { taskKey: task.key, projectId: task.projectId },
    });
    if (!opened) {
      navigate.toPluginPanel(PANEL_PATH, {
        subPath: taskDetailSubPath(task.key, task.projectId),
      });
    }
  };
...
function TaskEmbedPanelContent({ params }: PluginThreadPanelProps) {
  const taskKey =
    isRecord(params) && typeof params.taskKey === "string"
      ...
      : null;
  const projectId =
    isRecord(params) && typeof params.projectId === "string"
      ? params.projectId
      : undefined;
