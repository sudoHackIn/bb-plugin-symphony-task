// Provider brand marks, vendored from `apps/app/src/components/icons/*`.
//
// The app's canonical provider-logo resolver (`apps/app/src/lib/provider-icon.ts`)
// and its brand SVGs live inside the app bundle, which a plugin build cannot
// import across the app/plugin boundary. Built-in providers expose no served
// `logoUrl` (only custom ACP agents do), so the glyphs have to be reproduced
// here. Each mark renders in `currentColor` so it follows the surrounding
// theme color in light and dark, exactly like the app copies. Keep these in
// sync with the app icons if a brand mark changes.
