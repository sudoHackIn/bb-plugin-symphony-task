...
export const tasksRpcContract = defineRpcContract({
    ...
    input: z
      .object({
        ...
        linkedBbProjectId: z
          ...
          .nullable()
          .default(null),
        // Optional for backwards compatibility with the original Tasks CLI.
        // The plugin UI always supplies an explicit source for new projects.
        taskProvider: z.enum(["local", "beads", "jira"]).optional(),
  ...
  getTask: {
    input: z
      .object({ taskId: idSchema, projectId: idSchema.optional() })
      .strict(),
  ...
  getTaskByKey: {
    input: z
      .object({ taskKey: nonBlankStringSchema, projectId: idSchema.optional() })
      .strict(),
    output: z.object({ task: taskSchema.nullable() }).strict(),
  },
  updateTask: {
  ...
  attachTaskThread: {
    input: z
      .object({ taskId: idSchema, threadId: z.string().startsWith("thr_") })
      .strict(),
    output: z.object({ taskThread: taskThreadSchema }).strict(),
  },
  detachTaskThread: {
    input: z
      .object({ taskId: idSchema, threadId: z.string().startsWith("thr_") })
  ...
  listTaskThreadsForTasks: {
    input: z
      .object({ taskIds: z.array(idSchema).max(500) })
      .strict(),
    output: z.object({ taskThreads: z.array(taskThreadSchema) }).strict(),
  },
